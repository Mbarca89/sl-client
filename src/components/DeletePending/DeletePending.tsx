...
interface DeletePendingProps {
    id: string;
    updateList: () => void;
}

const DeletePending: React.FC<DeletePendingProps> = ({ id, updateList }) => {
    ...
    const handleDelete = async () => {
        setloading(true)
        try {
            const res = await axiosWithToken.put(`${SERVER_URL}/api/pending/delete?id=${id}`)
            if (res.data) {
    ...
    return (
        <div className="d-flex flex-column align-items-center">
            <span>¿Esta seguro que quiere eliminar la tarea?</span>
