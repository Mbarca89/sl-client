...
const Guides = () => {
    ...
    const getGuides = async () => {
        try {
            ...
            if (user.role === "Administrador") {
                res = await axiosWithToken.get(`${SERVER_URL}/api/guides/getAllGuides`)
            } else {
                res = await axiosWithToken.get(`${SERVER_URL}/api/guides/getUserGuides`)
            }
            if (res.data) {
                setGuides(res.data)
            }
    ...
    return (
        <div className='container text-light flex-grow-1 p-lg-3 p-sm-0 rounded bg-dark-800 m-2 overflow-auto'>
            {user.role === "Administrador" && <div onClick={handleCreate} role="button" className="d-flex justify-content-start align-items-center">
            ...
            <div className="d-flex flex-column gap-3 w-100 mt-3">
                {guides.map((guide, index) => (
                    <div key={index} className="w-100 bg-light border rounded text-dark text-start position-relative d-flex align-items-center">
                        <div className="w-100 m-0 p-1" role="button" onClick={() => navigate(`/guide/${guide.id}`)}>
                            <p className="m-0"><b>{guide.title}</b></p>
                        </div>
                        <div>
                            {user.role === "Administrador" && <svg role="button" onClick={() => handleDelete(guide.id)} className="" width="40" height="40" viewBox="0 0 24 24" fill="#343C54" xmlns="http://www.w3.org/2000/svg">
            ...
            {show && modal === "create" &&
                <CustomModal title={"Crear guia"}>
                    <CreateGuide updateList={getGuides}></CreateGuide>
                </CustomModal>
            }
             {show && modal === "delete" &&
                <CustomModal title={"Eliminar guia"}>
                    <DeleteGuide updateList={getGuides} id={currentGuide}></DeleteGuide>
                </CustomModal>
            }
        </div>
    )
